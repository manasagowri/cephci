...
node("rhel-8-medium || ceph-qe-ci") {
    ...
    try {
        stage('prepareJenkinsAgent') {
            ...
            results = sharedLib.readFromResultsFile("16.2.10-92")
            results.each{runtype,runtypeval->
                runtypeval.each{tier, tierval->
                    tierval.each{stage, stageval->
                        testreslist = []
                        for (testres in stageval["test_results"]) {
                            println("testres : ${testres}")
                            if (testres["passed"] == "null" || testres["passed"] == null){
                                println("Inside if")
                                testres["passed"] = 0
                            }
                            testreslist.add(testres)
                        }
                        results[runtype][tier][stage]["test_results"] = testreslist
                    }
                }
            }
            def resultsJson = writeJSON returnText: true, json: results
            def cmd = "cd ${env.WORKSPACE}/pipeline/scripts/ci;"
            cmd = "${cmd} sudo ${env.WORKSPACE}/.venv/bin/python update_results.py"
            cmd = "${cmd} --cephVersion 16.2.10-92"
            cmd = "${cmd} --testResults '${resultsJson}'"

            println("cmd : ${cmd}")

            sh (returnStdout: false, script: cmd)
//             sh (returnStdout: false, script: "sudo rm -rf /ceph/cephci-jenkins/results/17.2.5-49_test.yaml")
        }
...
//         }
        ...
        println("failureReason : ${failureReason}")
