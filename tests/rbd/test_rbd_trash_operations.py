...
def test_rbd_trash_operations_for_pool(
    ...
    try:
        ...
        for index, image in enumerate(images_without_snaps_and_clones):
            # if index % 2 == 0:
            # pdb.set_trace()
            minutes_add = 5 * (index+1)
            snap_spec = {f"{image}_snap": [f"{image}_clone"]}
            req_date = curr_time + timedelta(minutes=minutes_add)
        ...
        def convert_str_to_time(time):
            ...
            parsed_datetime = datetime.strptime(" ".join(time_components[:-2])+ " "+ time_components[-1], "%a %b %d %H:%M:%S %Y")
            parsed_timezone = pytz.timezone(time_components[-2])
            parsed_datetime = parsed_timezone.localize(parsed_datetime)
            return parsed_datetime
            # datetime.strptime(time, "%a %b %d %H:%M:%S %Z %Y")
        
        images_to_be_purged = [
            image
            for image, im_config in image_config.items()
            if convert_str_to_time(im_config.get("expires-at"))
            < date_before
        ]
        pdb.set_trace()
        rc = purge_images_and_verify(
        ...
        images_restored = [image for image in images_without_snaps_and_clones if image not in images_to_be_purged]
        rc = restore_and_create_new_image(
            rbd,
            pool,
            pool_type,
