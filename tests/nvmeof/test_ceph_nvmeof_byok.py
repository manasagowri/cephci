...
ENCRYPT_SETTLE_SEC = 15
CLONE_NS_ADD_TRIES = 4
CLONE_NS_ADD_DELAY = 10
...
def _ns_add(gateway, args):
    ...
    try:
        ...
        if _rbd_image_already_exists(exc) and add_args.get("rbd-create-image"):
            ...
            return gateway.namespace.add(**{"args": add_args})
        if _is_wrong_passphrase(exc):
            raise RuntimeError(str(exc)) from exc
        raise


def _is_wrong_passphrase(exc):
    return "wrong passphrase" in str(exc).lower()


def _ns_add_clone(
    gateway, args, tries=CLONE_NS_ADD_TRIES, delay=CLONE_NS_ADD_DELAY
):
    """Add a stacked clone namespace, retrying mislabeled lock errors.

    The gateway maps librbd ``PermissionError`` (including exclusive-lock
    races right after ``rbd encryption format``) to ``Wrong passphrase``.
    """
    last_exc = None
    for attempt in range(1, tries + 1):
        try:
            return _ns_add(gateway, args)
        except RuntimeError as exc:
            if not _is_wrong_passphrase(exc):
                raise
            last_exc = exc
            LOG.warning(
                "clone ns add %s wrong passphrase attempt %s/%s",
                args.get("rbd_image_name"),
                attempt,
                tries,
            )
            if attempt == tries:
...
def _encrypt_rbd_image(rbd_obj, pool, image, fmt, passphrase_file):
    """Apply ``rbd encryption format`` to an image; tolerate already-formatted."""
    result = rbd_obj.exec_cmd(
        cmd=f"rbd encryption format {pool}/{image} {fmt} {passphrase_file}",
        all=True,
    )
    ...
    if out == 1:
        ...
        if any(token in text for token in ("already encrypted", "already formatted")):
            LOG.info(f"{pool}/{image} already encrypted; continuing")
            return passphrase_file
        raise RuntimeError(
            f"rbd encryption format failed for {pool}/{image}: {err}"
        )
    LOG.info(f"Applied {fmt} encryption to {pool}/{image}")
    LOG.info(
        "Waiting %ss after encryption format for exclusive lock to settle",
        ENCRYPT_SETTLE_SEC,
    )
    time.sleep(ENCRYPT_SETTLE_SEC)
    return passphrase_file
...
def _encrypt_clone(rbd_obj, pool, clone_image, clone_fmt, clone_key, passphrase_file=None):
    ...
    return _encrypt_rbd_image(
        rbd_obj, pool, clone_image, clone_fmt, passphrase_file
    )
...
def run(ceph_cluster: Ceph, **kwargs) -> int:
    ...
    try:
        ...
        if config.get("skip_clones"):
            LOG.info("skip_clones: not adding clone namespaces")
            for sub in subsystems:
                sub.setdefault("clones", [])
        else:
