...
def parse_kmip_get_value(output):
    ...
    Dummy kmip-cli often returns ``value_preview`` truncated to ~35
    characters with a trailing ``...``. Callers must treat that as a
    prefix, not the full secret (see ``_kmip_value_matches``).
    """
    text = str(output or "").strip()
    if not text:
        return None
...
def kmip_cli(
    ...
    Args:
        ...
        port: Optional ``--port`` when the server is not on 5696 (HA standby).
        certs_dir: Host cert directory mounted at ``/kmip/certs``. Defaults to
            ``KMIP_DATA_CERTS`` on this node.
    """
    host_arg = ""
    if hostname:
        host_arg = f"--hostname {hostname} "
    port_arg = f"--port {port} " if port else ""
    certs = certs_dir or KMIP_DATA_CERTS
    cmd = (
        f"podman run --rm --network host "
        f"-v {certs}:/kmip/certs:ro,Z "
        f"{cli_image} {host_arg}{port_arg}{args}"
    )
...
def destroy_passphrase(
    ...
    """Destroy a KMIP uuid. Missing objects are not a failure by default."""
    hostname = hostname or node.ip_address
    LOG.info("[%s] kmip-cli destroy uuid=%s", short_hostname(node), uuid)
    return kmip_cli(
        node,
        f"destroy --uuid {uuid}",
        cli_image=cli_image,
...
def _kmip_objects(
    ...
    hostname = hostname or node.ip_address
    objects = []
    for uuid in _list_kmip_uuids(
        node, cli_image=cli_image, port=port, certs_dir=certs_dir, hostname=hostname
    ):
...
def _kmip_value_matches(fetched, expected):
    """Return True if a kmip-cli get result matches the expected secret.

    Exact match first. Dummy kmip-cli ``value_preview`` truncates long
    secrets (OpenStack hostnames such as ``passwd_<vm-host>_1``) to ~35
    characters and appends ``...``. A leftover short secret such as
    ``passwd`` must not match a longer ``passwd_<host>_1`` value.
    """
    fetched = str(fetched or "")
    expected = str(expected or "")
    if fetched == expected:
        return True
    if not fetched or not expected or len(fetched) >= len(expected):
        return False
    prefix = fetched[:-3] if fetched.endswith("...") else fetched
    if not prefix or not expected.startswith(prefix):
        return False
    return fetched.endswith("...") or len(fetched) >= 32
...
def _find_kmip_uuid(objects, spec):
    ...
    If the name is unique and the stored value is only a truncated
    preview, accept that uuid so clone IO can reload parent key-ids
    created on long VM hostnames.
    """
    named = [obj for obj in objects if obj.get("name") == spec["name"]]
    for obj in named:
        if _kmip_value_matches(obj.get("value"), spec["value"]):
            return obj["uuid"]
    if len(named) == 1:
        LOG.warning(
            f"KMIP name {spec['name']!r} is unique but value preview "
            f"{secret_log_id(named[0].get('value'))!r} did not match "
            f"{secret_log_id(spec['value'])!r}; using uuid={named[0]['uuid']}"
        )
        return named[0]["uuid"]
    return None
