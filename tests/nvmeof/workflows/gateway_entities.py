...
from ceph.nvmeof.initiators.linux.nvme_cli import initiators, run_io
...
def manage_subsystems(config, _cls, command):
    """
    Manage subsystems across all gateways in the NVMe service.
    Args:
        config: subsystem configuration
        _cls: NVMeGateway class
        command: Command to execute on subsystems, e.g., add, delete
    """
    max_ns = config["args"].pop("max-namespaces")

    def manage_subsystem(num):
    ...
    with parallel() as p:
        for num in range(1, config["args"].pop("subsystems") + 1):
            p.spawn(manage_subsystem, num)


def manage_listeners(config, _cls, command, ceph_cluster):
    """Manage listeners across all gateways in the NVMe service.
    Args:
        config: listener configuration
        _cls: NVMeGateway class
        command: Command to execute on listeners, e.g., add, delete
        ceph_cluster: Ceph cluster instance
    """
    ...
    def manage_listener(num, listener_node):
        subnqn = (
    ...
    with parallel() as p:
        for node in nodes:
            ...
            for num in range(1, subsystems + 1):
                p.spawn(manage_listener, num, listener_node)


def manage_hosts(config, _cls, command):
    """Manage hosts across all gateways in the NVMe service.
    Args:
        config: host configuration
        _cls: NVMeGateway class
        command: Command to execute on hosts, e.g., add, delete"""
...
def manage_namespaces(
    ...
    if command == "add":
        for sub_num in range(1, subsystems + 1):
            ...
            def manage_namespace(num, config):
            ...
            # Manage namespaces in parallel
            for num in range(1, namespaces_sub + 1):
                with parallel() as p:
                    p.spawn(manage_namespace, num, config)
