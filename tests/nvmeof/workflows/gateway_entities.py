        ...
        if sub_cfg.get("listeners"):
            ...
            listener_config = {
                "args": {
                    ...
                    "host-name": getattr(gateway.node, "hostname", str(gateway.node)),
                }
            }
    ...
    for d in daemons:
        ...
        for gateway in nvme_service.gateways:
            if subsystem_config:
                ...
                configure_namespaces(gateway, nvme_service.config, rbd_obj=rbd_obj)
...
def teardown(nvme_service, rbd_obj):
    ...
    if "subsystems" in nvme_service.config["cleanup"]:
        ...
        for sub_cfg in config_sub_node:
            ...
            gateway = nvme_service.gateways[0]
            err, out = gateway.subsystem.delete(
