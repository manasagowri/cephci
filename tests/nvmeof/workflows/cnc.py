...
def measure_cnc_throughput(bytes_copied, elapsed):
    """Return throughput in MiB/s for bytes_copied over elapsed seconds."""
    if elapsed <= 0:
        raise ValueError(f"elapsed must be > 0, got {elapsed}")
    mib_s = (bytes_copied / (1024 * 1024)) / elapsed
    LOG.info(
        f"CNC throughput: {bytes_copied} bytes in {elapsed:.2f}s "
        f"= {mib_s:.3f} MiB/s"
    )
    return mib_s
...
def run_concurrent_cnc_copies(
    ...
    """Run N non-overlapping format-2 copies concurrently.

    Args:
        ranges: iterable of (src_slba, dst_slba, blocks)
        mcl: namespace max copy length (0-based NLB); uses copy_within_mcl

    Returns:
        dict with elapsed wall time, bytes_copied (requires lba_size externally),
        and per-range elapsed list.
    """
    ranges = list(ranges)
    workers = max_workers or max(1, len(ranges))
    per_elapsed = [None] * len(ranges)
    ...
    with parallel(max_workers=workers) as p:
        for idx, (src_slba, dst_slba, blocks) in enumerate(ranges):
            p.spawn(_one, idx, src_slba, dst_slba, blocks)
...
def run_rate_limiter_enforcement(initiator, gateways, config=None):
    """Prove rate-limit-bytes caps concurrent CNC vs unlimited baseline."""
    config = config or {}
    devices = get_ns_devices(initiator)
    caps = assert_copy_capabilities(initiator, devices)
    ...
    # Large per-copy size; copy_within_mcl splits when exceeding mcl
    copy_blocks = int(config.get("copy_blocks", 65536))
    rate_limited = int(config.get("rate_limit_bytes", 1048576))
    rate_unlimited = int(config.get("unlimited_rate_limit_bytes", 400000000))
    max_limited_MBps = float(config.get("max_limited_MBps", 3))
    min_slowdown = config.get("min_slowdown")
    base_cfg = {
        "host_behav_support_cnc": True,
        "chunk_nlb": 512,
        "max_inflight": 8,
        **(config.get("cnc_config") or {}),
    }
    ...
    LOG.info(
        f"Preparing {prep_bytes} bytes source for {concurrent} x "
    ...
    def _run_phase(rate_limit_bytes, label):
        apply_cnc_config(
            gateways,
            cnc_config={**base_cfg, "rate_limit_bytes": rate_limit_bytes},
            enable_logging=config.get("enable_cnc_logging", True),
        )
        sample_gateway_resources(gateways)
        ...
        sample_gateway_resources(gateways)
        LOG.info(
            f"Rate limiter {label}: rate_limit_bytes={rate_limit_bytes} "
            f"throughput={mib_s:.3f} MiB/s elapsed={result['elapsed']:.2f}s"
        )
        return {"elapsed": result["elapsed"], "mib_s": mib_s, "bytes": bytes_copied}

    limited = _run_phase(rate_limited, "limited")
    unlimited = _run_phase(rate_unlimited, "unlimited")

    if limited["mib_s"] > max_limited_MBps:
        raise Exception(
            f"Limited-path throughput {limited['mib_s']:.3f} MiB/s exceeds "
            f"max_limited_MBps={max_limited_MBps} (cap ~"
            f"{rate_limited / (1024 * 1024):.3f} MiB/s)"
        )

    if unlimited["elapsed"] <= 0:
        raise Exception("Unlimited phase reported non-positive elapsed time")
    slowdown = limited["elapsed"] / unlimited["elapsed"]
    LOG.info(
        f"Rate limiter slowdown ratio limited/unlimited elapsed="
        f"{slowdown:.2f}x "
        f"(T_lim={limited['elapsed']:.2f}s T_unlim={unlimited['elapsed']:.2f}s)"
    )
    if min_slowdown is not None and slowdown < float(min_slowdown):
        raise Exception(
            f"Expected limited path >= {min_slowdown}x slower than unlimited, "
            f"got {slowdown:.2f}x"
        )
...
def run_fio_parallel_cnc(initiator, gateways, config=None):
    """Host fio on one LBA region while CNC copies another in parallel."""
    config = config or {}
    devices = get_ns_devices(initiator)
    caps = assert_copy_capabilities(initiator, devices)
    apply_cnc_config(gateways, cnc_config=config.get("cnc_config"))
    ...
    fio_bs = config.get("fio_bs", "1M")
    fio_iodepth = int(config.get("fio_iodepth", 32))
    fio_runtime = config.get("fio_runtime")
    cnc_loop_count = int(config.get("cnc_loop_count", 50))
    cnc_blocks = min(int(config.get("cnc_blocks", BLOCKS_4MIB)), mcl + 1)

    fio_bytes = _parse_size(fio_size)
    fio_blocks = fio_bytes // lba_size
    # Non-overlapping: fio at low LBAs; CNC source starts after fio region + gap
    cnc_src_base = fio_blocks + (1024 * 1024 // lba_size)  # +1MiB gap
    cnc_prep_bytes = cnc_loop_count * cnc_blocks * lba_size

    LOG.info(
        f"Partitioning: fio [0, {fio_bytes}) on {src_dev}; "
        f"CNC source base LBA={cnc_src_base} size={cnc_prep_bytes}"
    )
    write_verified_pattern(
        node,
        src_dev,
        size=cnc_prep_bytes,
        offset=cnc_src_base * lba_size,
        verify="crc32c",
        bs=fio_bs,
        iodepth=fio_iodepth,
    )
    ...
    def _fio_workload():
        try:
            # Size-based write+read verify (default); optional time_based overlay
            write_verified_pattern(
                node,
                src_dev,
                size=fio_size,
            ...
            if fio_runtime:
                # Extra timed randrw while CNC may still be running
                run_fio(
                    device_name=src_dev,
                    client_node=node,
                    io_type="randrw",
                    ...
                    verify="crc32c",
                    verify_fatal=1,
                    run_time=int(fio_runtime),
                    long_running=True,
                    cmd_timeout="notimeout",
                    test_name=f"cnc-fio-parallel-{src_dev.replace('/', '_')}",
                )
            write_verified_pattern(
                node,
                src_dev,
    ...
    def _cnc_workload():
        try:
            run_cnc_loop(
                ...
                dst_dev,
                src_nsid,
                count=cnc_loop_count,
                blocks=cnc_blocks,
                src_base_slba=cnc_src_base,
                dest_base_slba=0,
                mcl=mcl,
            )
        except Exception as err:
            cnc_errors.append(err)
            raise

    sample_gateway_resources(gateways)
    with parallel(max_workers=2) as p:
        p.spawn(_fio_workload)
        p.spawn(_cnc_workload)
    sample_gateway_resources(gateways)

    if fio_errors or cnc_errors:
        raise Exception(f"Parallel fio/CNC failed: fio={fio_errors} cnc={cnc_errors}")
    ...
    # Final fio-region read verify (ensure CNC did not corrupt low LBAs)
    write_verified_pattern(
        node,
        src_dev,
        size=fio_size,
    ...
    LOG.info("Parallel fio + CNC completed with data verification")
...
OPERATIONS = {
    ...
    "rate_limiter_enforcement": run_rate_limiter_enforcement,
    "fio_parallel_cnc": run_fio_parallel_cnc,
}
