...
def test_trash_primary_image(rbd_mirror, rbd, pool_type, **kw):
    try:
...
def run(**kw):
    ...
    if mirror_obj:
        ...
        if test_trash_primary_image(
            mirror_obj.get("rep_rbdmirror"), mirror_obj.get("rep_rbd"), "rep_pool_config", **kw
        ):
            return 1

        log.info("Executing test on ec pool")
        if test_trash_primary_image(
            mirror_obj.get("ec_rbdmirror"), mirror_obj.get("ec_rbd"), "ec_pool_config", **kw
        ):
            return 1
